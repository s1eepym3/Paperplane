  ...
  try {
    ...
    if (token === 'demo') {
      ...
      try {
        ...
        console.error('Failed to send demo acceptance email:', emailError);
      }
    ...
    // Send email notification (non-blocking for UX, but awaited to prevent early termination on serverless Vercel)
    try {
      const origin = request.headers.get('origin') || new URL(request.url).origin;
      ...
      console.error('Failed to send acceptance email notification:', emailError);
    }
